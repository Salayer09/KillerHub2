...
local SheriffConfig = {
    ...
    HorizontalPred = 0.152, 
    VerticalPred = 0.042,   
    WallCheck = true,    
    CloseRangeZone = 7, 
    AntiBaiting = true, 
    HitrateEnhancer = true,
    AutoUnequip = false,  -- Recuperado de la v6.7.2
...
SheriffTab:CreateToggle("HitrateEnhancerToggle", "Optimizar Balística Predictiva", function(estado) SheriffConfig.HitrateEnhancer = estado saveConfig() end)
...
SheriffTab:CreateToggle("AntiBaitingToggle", "Filtro Anti-Amague (Anti-Baiting)", function(estado) SheriffConfig.AntiBaiting = estado saveConfig() end)
SheriffTab:CreateDropdown("PredMode", "Modo de Predicción:", {"Híbrido Absoluto (Omni)", "Predictiva 2.0 (Aceleración)", "Predictivo Adaptativo"}, function(seleccionado) SheriffConfig.PredictionMode = seleccionado saveConfig() end)
SheriffTab:CreateSlider("HorizontalPredSlider", "Predicción Horizontal", 0, 300, function(valor) SheriffConfig.HorizontalPred = valor / 1000 saveConfig() end)
SheriffTab:CreateSlider("VerticalPredSlider", "Predicción Vertical (Suave)", 0, 120, function(valor) SheriffConfig.VerticalPred = valor / 1000 saveConfig() end)
SheriffTab:CreateSlider("CloseRangeZoneSlider", "Zona Muerta Quemarropa (Studs)", 0, 20, function(valor) SheriffConfig.CloseRangeZone = valor saveConfig() end)

SheriffTab:CreateSection("Líneas de Trayectoria")
SheriffTab:CreateMultiDropdown("ActiveTracers", "Seleccionar Tracers Activos:", {"Impacto Final (Rojo)", "Ping (Azul)", "Lag (Violeta)", "Lead (Verde)"}, function(tablaFlags)
...
SheriffTab:CreateSlider("LeadTimeSlider", "Anticipación de la Mano (Lead Time)", 0, 100, function(valor) SheriffConfig.LeadTimePred = valor / 100 saveConfig() end)

SheriffTab:CreateSection("Ajustes de Interfaz / Tácticas")
SheriffTab:CreateToggle("WeaponDetectToggle", "Ocultar Botón si no tengo Arma en Inventario", function(estado) SheriffConfig.UseWeaponDetector = estado saveConfig() end)
SheriffTab:CreateToggle("AutoUnequipToggle", "Auto-Desequipar Arma (Fast Unequip)", function(estado) SheriffConfig.AutoUnequip = estado saveConfig() end)
SheriffTab:CreateToggle("ShowVoidButton", "Mostrar Botón en Pantalla", function(estado) SheriffConfig.ShowShootButton = estado end)
...
SheriffTab:CreateSlider("VoidBtnSize", "Tamaño del Botón Sheriff", 50, 200, function(valor)
    ...
    if btn then btn.Size = UDim2.new(0, valor, 0, valor) if btn:FindFirstChild("UICorner") then btn.UICorner.CornerRadius = UDim.new(0, math.floor(valor * 0.28)) end end
end)
...
SheriffTab:CreateToggle("LockVoidBtn", "Bloquear Posición del Botón", function(estado) SheriffConfig.ButtonLocked = estado end)

-- MOTOR CINEMÁTICO ADAPTATIVO
...
local function getSmoothedPing(rawPing)
    ...
    for _, p in ipairs(pingHistory) do sum = sum + p if p > maxRecentPing then maxRecentPing = p end end
...
if RoundStart and RoundStart:IsA("RemoteEvent") then
    RoundStart.OnClientEvent:Connect(function(arg1, arg2)
        table.clear(playerRoles) table.clear(playerDeadStatus) parsePlayerData(arg2) parsePlayerData(arg1)
    end)
end
...
local function getMurderer()
    if currentTarget and currentTarget.Parent and currentTarget.Character then
        ...
        if (playerRoles[name] == "Murderer" or hasKnife) and not isDead then return currentTarget end
    end
    for name, role in pairs(playerRoles) do
        if role == "Murderer" then
            ...
            if pl and pl.Character and pl ~= LocalPlayer then
                ...
                if not ((hum and hum.Health <= 0) or (playerDeadStatus[name] == true)) then currentTarget = pl return pl end
            end
        end
    end
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Parent ~= nil then
            ...
            if char and char:FindFirstChild("Knife") then 
                ...
                if not ((char:FindFirstChildOfClass("Humanoid") and char:FindFirstChildOfClass("Humanoid").Health <= 0) or (playerDeadStatus[name] == true)) then currentTarget = player return player end
            end
        end
    end
    return nil
...
-- WALL CHECK CORREGIDO HÍBRIDO (No se bugea si el arma está en mochila)
...
local function isTargetVisible(targetPart, murdererChar)
    ...
    -- SOLUCIÓN: Si el arma está equipada usa el cañón, si está guardada calcula desde nuestro Torso (como la v6.7.2)
    ...
    local wallCheckRay = workspace:Raycast(originPos, direction, wallCastParams)
    
    if wallCheckRay and wallCheckRay.Instance.CanCollide == true then return false end
    return true
end
...
local function getPredictedPosition(targetChar, targetPart, customDelta)
    ...
    if distance <= limiteCerca then factorRangoLerp = 0 elseif distance < limiteLejos then factorRangoLerp = (distance - limiteCerca) / (limiteLejos - limiteCerca) end

    local predictionWeight = 1 * factorRangoLerp
    local minZone = SheriffConfig.CloseRangeZone
    local maxZone = minZone + 12
    if distance <= minZone then predictionWeight = 0 elseif distance < maxZone and minZone ~= maxZone then predictionWeight = ((distance - minZone) / (maxZone - minZone)) * factorRangoLerp end

    if lastTargetChar ~= targetChar then smoothedVelocity = rawVelocity previousTargetVelocity = smoothedVelocity lastRawVelocity = rawVelocity lastTargetChar = targetChar end
    ...
    if dotProduct < 0.65 and SheriffConfig.AntiBaiting then baitingFactor = math.clamp((dotProduct + 1) / 2.3, 0.1, 0.5) end

    local clampedDT = math.min(activeDT, 0.05) 
    local responseSpeed = (cachedPingValue > 0.11) and 10.5 or 15.0 
    ...
    -- AJUSTADO: Sincronización exacta con la velocidad máxima real de MM2 (16.715)
    local speedFactor = math.clamp(smoothedVelocity.Magnitude / 16.715, 0, 1.25)
    ...
    if humanoid.FloorMaterial == Enum.Material.Air or math.abs(smoothedVelocity.Y) > 0.1 then
        ...
        if smoothedVelocity.Y > 0.5 then pY = pY + (smoothedVelocity.Y * 0.006 * predictionWeight) end
        verticalOffset = Vector3.new(0, pY, 0)
    end

    local finalPrediction = targetPosition + finalHorizontal + verticalOffset
    local pingPrediction  = targetPosition + pingHorizontal + verticalOffset
    local lagPrediction   = targetPosition + lagHorizontal + verticalOffset
...
-- ASIGNACIÓN DE TRACERS VISUALES
local LagLine = Drawing.new("Line") LagLine.Color = Color3.fromRGB(150, 50, 255) LagLine.Thickness = 1.1 LagLine.ZIndex = 2 LagLine.Visible = false table.insert(_G.KillerHubLines, LagLine)
local PingLine = Drawing.new("Line") PingLine.Color = Color3.fromRGB(0, 100, 255) PingLine.Thickness = 1.1 PingLine.ZIndex = 3 PingLine.Visible = false table.insert(_G.KillerHubLines, PingLine)
local LeadLine = Drawing.new("Line") LeadLine.Color = Color3.fromRGB(0, 255, 100) LeadLine.Thickness = 1.1 LeadLine.ZIndex = 4 LeadLine.Visible = false table.insert(_G.KillerHubLines, LeadLine)
local PredictionLine = Drawing.new("Line") PredictionLine.Color = Color3.fromRGB(255, 35, 35) PredictionLine.Thickness = 1.5 PredictionLine.ZIndex = 5 PredictionLine.Visible = false table.insert(_G.KillerHubLines, PredictionLine)

local currentScreenPred, currentScreenPing, currentScreenLag, currentScreenLead = Vector2.new(0,0), Vector2.new(0,0), Vector2.new(0,0), Vector2.new(0,0)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if not murderer or not murderer.Character then PredictionLine.Visible = false; PingLine.Visible = false; LagLine.Visible = false; LeadLine.Visible = false; firstFrame = true return end

    local targetChar = murderer.Character
    local bestPart = targetChar:FindFirstChild("HumanoidRootPart") or targetChar:FindFirstChild("Head")
    if bestPart then
        local localHrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        local originPos = localHrp and localHrp.Position or Camera.CFrame.Position
        local distance = (bestPart.Position - originPos).Magnitude
        ...
        if lagPos and SheriffConfig.ShowLagTracer then
            ...
            if onScreen then currentScreenLag = (firstFrame or tSmooth == 1) and vec2New(screenPos.X, screenPos.Y) or currentScreenLag:Lerp(vec2New(screenPos.X, screenPos.Y), tSmooth) LagLine.From = screenOrigin LagLine.To = currentScreenLag LagLine.Visible = true else LagLine.Visible = false end
        ...
        if pingPos and SheriffConfig.ShowPingTracer then
            ...
            if onScreen then currentScreenPing = (firstFrame or tSmooth == 1) and vec2New(screenPos.X, screenPos.Y) or currentScreenPing:Lerp(vec2New(screenPos.X, screenPos.Y), tSmooth) PingLine.From = screenOrigin PingLine.To = currentScreenPing PingLine.Visible = true else PingLine.Visible = false end
        ...
        if SheriffConfig.ShowLeadTracer then
            ...
            local leadPredictedPos = bestPart.Position + (vec3New(smoothedVelocity.X, smoothedVelocity.Y * 0.4, smoothedVelocity.Z) * SheriffConfig.LeadTimePred * distFactor)
            local targetScreenPos, targetOnScreen = worldToViewport(Camera, leadPredictedPos)
            if targetOnScreen then currentScreenLead = (firstFrame or tSmooth == 1) and vec2New(targetScreenPos.X, targetScreenPos.Y) or currentScreenLead:Lerp(vec2New(targetScreenPos.X, targetScreenPos.Y), tSmooth) LeadLine.From = (hand and worldToViewport(Camera, hand.Position)) and vec2New(worldToViewport(Camera, hand.Position).X, worldToViewport(Camera, hand.Position).Y) or screenOrigin LeadLine.To = currentScreenLead LeadLine.Visible = true else LeadLine.Visible = false end
        ...
        if predictedPos and SheriffConfig.PredictTracer then
            ...
            if onScreen then currentScreenPred = (firstFrame or tSmooth == 1) and vec2New(screenPos.X, screenPos.Y) or currentScreenPred:Lerp(vec2New(screenPos.X, screenPos.Y), tSmooth) PredictionLine.From = screenOrigin PredictionLine.To = currentScreenPred PredictionLine.Visible = true else PredictionLine.Visible = false end
        else PredictionLine.Visible = false end
        firstFrame = false
    else PredictionLine.Visible = false; PingLine.Visible = false; LagLine.Visible = false; LeadLine.Visible = false; firstFrame = true end 
end)
...
local function fireAtMurdererDirectly()
    ...
    if gun and murderer and murderer.Character then
        local targetChar = murderer.Character
        local bestPart = getBestTargetPart(targetChar) -- Ejecuta el Wall Check corregido sin depender del cañón guardado
        if bestPart then 
...
local Corner = Instance.new("UICorner") Corner.CornerRadius = UDim.new(0, math.floor(SheriffConfig.ButtonSize * 0.28)) Corner.Parent = ShootButton
local GlowOverlay = Instance.new("Frame") GlowOverlay.Name = "GlowOverlay" GlowOverlay.Size = UDim2.new(1, 0, 1, 0) GlowOverlay.BackgroundTransparency = 1 GlowOverlay.ZIndex = ShootButton.ZIndex + 1 GlowOverlay.Parent = ShootButton
local GlowCorner = Instance.new("UICorner") GlowCorner.CornerRadius = Corner.CornerRadius GlowCorner.Parent = GlowOverlay
local UiGradient = Instance.new("UIGradient") UiGradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(24, 8, 43)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(131, 46, 222)), ColorSequenceKeypoint.new(1, Color3.fromRGB(24, 8, 43))}) UiGradient.Rotation = 45 UiGradient.Parent = GlowOverlay
local DecalTexture = Instance.new("ImageLabel") DecalTexture.Name = "DecalTexture" DecalTexture.Size = UDim2.new(0.38, 0, 0.38, 0) DecalTexture.AnchorPoint = Vector2.new(0.5, 0.5) DecalTexture.Position = UDim2.new(0.5, 0, 0.43, 0) DecalTexture.BackgroundTransparency = 1 DecalTexture.Image = "rbxassetid://125754446555599" DecalTexture.ImageTransparency = 1 - SheriffConfig.ButtonOpacity DecalTexture.ZIndex = ShootButton.ZIndex + 2 DecalTexture.Parent = ShootButton
...
local function iniciarAnimacionIcono(decalTexture)
    ...
    tweenIda.Completed:Connect(function() task.wait(0.2) tweenVuelta:Play() end)
    tweenVuelta.Completed:Connect(function() task.wait(0.2) tweenIda:Play() end)
    tweenIda:Play()
end
iniciarAnimacionIcono(DecalTexture)

local Label = Instance.new("TextLabel") Label.Name = "Label" Label.Size = UDim2.new(1, 0, 0.2, 0) Label.Position = UDim2.new(0, 0, 0.75, 0) Label.BackgroundTransparency = 1 Label.Text = "SHOOT" Label.TextColor3 = Color3.fromRGB(255, 255, 255) Label.TextSize = 15 Label.Font = Enum.Font.GothamBold Label.TextTransparency = 1 - SheriffConfig.ButtonOpacity Label.ZIndex = ShootButton.ZIndex + 2 Label.Parent = ShootButton

ShootButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        local buttonAbsolutePos = ShootButton.AbsolutePosition local buttonSize = ShootButton.AbsoluteSize local localX = input.Position.X - buttonAbsolutePos.X local relX = (localX / buttonSize.X) - 0.5 UiGradient.Offset = Vector2.new(relX * 1.5, 0) TweenService:Create(GlowOverlay, TweenInfo.new(0.04, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.25}):Play()
        ...
        if not SheriffConfig.ButtonLocked then
            ...
            input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then dragging = false SheriffConfig.ButtonX = ShootButton.Position.X.Scale SheriffConfig.ButtonY = ShootButton.Position.Y.Scale saveConfig() end end)
        end
    end
end)
ShootButton.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then TweenService:Create(GlowOverlay, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play() end end)
ShootButton.InputChanged:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then dragInput = input end end)
UserInputService.InputChanged:Connect(function(input) if input == dragInput and dragging then local delta = input.Position - dragStart ShootButton.Position = UDim2.new(startPos.X.Scale + (delta.X / Camera.ViewportSize.X), 0, startPos.Y.Scale + (delta.Y / Camera.ViewportSize.Y), 0) end end)
...
if ClientServices then
    ...
    local function checkAndPredict(returnCFrame)
        local currentTime = os.clock() local hookDelta = currentTime - lastHookCallTime lastHookCallTime = currentTime
        local structuralDelta = math.clamp(hookDelta, 0.008, 0.033) local gun, _ = getGunLocation()
        if SheriffConfig.SilentAim and (not SheriffConfig.UseWeaponDetector or (gun ~= nil)) then
            local murderer = getMurderer()
            if murderer and murderer.Character then
                local targetChar = murderer.Character local bestPart = getBestTargetPart(targetChar)
                if bestPart then local predictedPos = getPredictedPosition(targetChar, bestPart, structuralDelta) if predictedPos then return returnCFrame and CFrame.new(predictedPos) or predictedPos end end
            end
        end
        return nil
    end
    WeaponService.GetTargetPosition = function(self, ...) local prediction = checkAndPredict(false) return prediction or oldGetTargetPosition(self, ...) end
    WeaponService.GetMouseTargetCFrame = function(self, ...) local prediction = checkAndPredict(true) return prediction or oldGetMouseTargetCFrame(self, ...) end
end
